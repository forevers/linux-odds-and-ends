...
// TODO move into seperated headers
...
// requires Symbol: REGMAP_I2C [=m] in kernel config
#define USE_REGMAP

// static const struct regmap_config regmap_configuration_ __initconst = {
const struct regmap_config regmap_configuration_ = {
    .name           = "ess oled map",
    .reg_bits       = 8,
    .val_bits       = 8,
    .max_register	= SET_VCOM_DESEL,
};
...
static void vline(ess_ssd1305_oled* oled, __u8 x, __u8 y, __u8 height, __u8 creolor);
...
#if defined(USE_REGMAP)
    ret = regmap_bulk_write(oled->i2c_spi_regmap, SET_DISP_START_LINE, &oled->oled_data.buffer[1], len);
#else
    ret = i2c_master_send(oled->client, oled->oled_data.buffer, len);
#endif
    if (ret != len) {
        PR_ERR("i2c block write failure");
        dev_err(&oled->client->dev, "couldn't send I2C block data \n");
        return ret;
    }
...
#if defined(USE_REGMAP)
    return regmap_write(ssd1305_oled->i2c_spi_regmap, COMMAND, cmd);
#else
...
static int ess_oled_probe(struct i2c_client* client, const struct i2c_device_id* id)
{
    ...
    */
    ...
    // struct regmap_config regmap_configuration;
    ...
    /* init regmap */
    ess_ssd1305_oled_->i2c_spi_regmap = regmap_init_i2c(client, &regmap_configuration_);
    if (IS_ERR(ess_ssd1305_oled_->i2c_spi_regmap)) {
        PR_ERR("regmap_init_i2c() failure");
        kfree(ess_ssd1305_oled_->oled_data.buffer);
        kfree(ess_ssd1305_oled_);
        gpio_oled_remove(&client->dev);
        return PTR_ERR(ess_ssd1305_oled_->i2c_spi_regmap);
    }
...
static int ess_oled_remove(struct i2c_client *client)
{
    ...
    regmap_exit(ess_ssd1305_oled_->i2c_spi_regmap);
